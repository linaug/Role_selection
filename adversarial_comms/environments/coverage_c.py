...
class Robot():
    ...
    def update_state(self):
        ...
        # 计算漏点率
        uncover_in_explored = np.sum((interesting_map==1)) # 已经探索区域内的未覆盖的兴趣点之和
        explored = np.sum((self.world.map.exploration==0)) # 已探索区域大小
        loudian = uncover_in_explored/explored

        return self.state, self.reward, self.reward_coverage,self.reward_explore, loudian, self.explored_area,self.duplicate_coverage_reward, done, {}
...
class CoverageEnv(gym.Env, EzPickle):
    ...
    def step(self, actions):
        ...
        states, rewards, rewards_coverage,rewards_explore, loudians, duplicate_coverage_rewards,explored_areas = {}, {}, {}, {},{},{},{}
        for team_key, team in self.teams.items():
            states[team_key] = []
            rewards[team_key] = {}
            ...
            for i, agent in enumerate(team):
                state, reward, reward_coverage,reward_explore, loudian,explored_area, duplicate_coverage_reward, done, _ = agent.update_state()
        ...
        if Greedy: # greedy方法应该在全局地图上mask掉未被探索的区域
            map_obstacles_ = np.where(self.map.exploration==0,self.map.map,0)

        # 依据探索情况更新兴趣点覆盖全情况
            interesting_map_ = np.where(self.map.exploration==0,self.map.interesting,0)

            frontier_map_ = np.where(self.map.exploration==0,self.map.frontier,0)

            global_state = np.concatenate([np.stack([map_obstacles_, frontier_map_, interesting_map_], axis=-1), pose_map], axis=-1)
        # global_state = np.concatenate([np.stack([self.map.map, self.map.frontier, self.map.interesting], axis=-1), pose_map], axis=-1)
        # print('*-'*80)
        # print('obsctacle_shape is {} and froniter_shape is {} and interesting_shape is {} and pose_map_shape is {}'.format(self.map.map.shape, self.map.frontier.shape, self.map.interesting.shape,pose_map.shape))
        
        # print('*'*80)
        # print('state_shape_map:',states[1][0].shape)
        # print('state_shape_pos:',self.teams[1][0].pose)
        # print('state_shape_gso:',self.compute_gso(0).shape)
        # print('state_shape_state:',global_state.shape)
        ...
        flattened_rewards,flattened_rewards_explore,flattened_duplicate_coverage_rewards,flattened_loudian_ratio,flattened_explored_areas,flattened_rewards_coverage = {},{},{},{},{},{}
        agent_index = 0
        for key in self.teams.keys():
            for r,r_coverage,r_explore,r_duplicate, r_loudian, explored_ in zip(rewards[key].values(),
