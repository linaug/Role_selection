...
# from ..evaluate_greedy import traditional_plan
...
class Robot():
    ...
    def get_primitive_action(self,role_action,obs, plan):
        action=[]
        action.append(random.randrange(0, 5))
        start=[4,4]
        next_pose_list=[[-2,-2],]
        current_pose_list=[[-2,-2],]
        actions=plan.A_star(obs)
        pose_global = self.pose
        next_global=[pose_global[0]+plan.neighbors[actions][0],pose_global[1]+plan.neighbors[actions][1]]
        primitive_action = 0

        return primitive_action


    def step(self, role_action):
        # obs, _, _, _, _  = self.update_state()
        # plan = traditional_plan()
        # primitive_action = self.get_primitive_action(role_action,obs, plan)
    ...
    def update_state(self):
        ...
        return self.state, self.reward, self.duplicate_coverage_reward, done, {}
...
class CoverageEnv(gym.Env, EzPickle):
    ...
    def step(self, actions):
        ...
        states, rewards, duplicate_coverage_rewards, rewards_role = {}, {}, {},{}
        for team_key, team in self.teams.items():
            states[team_key] = []
            rewards[team_key] = {}
            ...
            for i, agent in enumerate(team):
                state, reward, duplicate_coverage_reward, done, _ = agent.update_state()
                states[team_key].append(state)
                rewards[team_key][i] = reward
               
                j = i if team_key == 0 else i+1                
                if actions == 0:
                    rewards_role[team_key][j] = self.cfg['ALPHA'] * reward
                else:
                    rewards_role[team_key][j] = self.cfg['BETA'] * reward
        ...
        flattened_rewards = {}
        flattened_rewards_role = {}
        agent_index = 0
        for key in self.teams.keys():
            for r_role, r in zip(rewards_role[key].values(),rewards[key].values()):
                flattened_rewards_role[agent_index] = r_role
                flattened_rewards[agent_index] = r
                agent_index += 1

        flattened_duplicate_coverage_rewards = {}
        agent_index_ = 0
        for key in self.teams.keys():
            for r in duplicate_coverage_rewards[key].values():
                flattened_duplicate_coverage_rewards[agent_index_] = r
                agent_index_ += 1
        ...
        info = {
            ...
            'role_rewards': flattened_rewards_role,
